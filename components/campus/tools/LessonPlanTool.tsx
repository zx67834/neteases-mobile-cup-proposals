            ...
            <li key={lesson.id}>
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {[lesson.subject, lesson.grade, formatDay(lesson.updated_at || lesson.created_at)]
                      .filter(Boolean)
                      .join(' · ')}
                  </p>
                </div>
    ...
    >
      <ToolCard>
            ...
            <li key={lesson.id}>
              ...
              >
                <div className="min-w-0">
                  <p className="font-medium text-slate-900 dark:text-slate-50">{lesson.title}</p>
                  <p className="mt-1 text-xs text-slate-500">
                    {[lesson.teacher_name, lesson.subject, lesson.grade, formatDay(lesson.updated_at)]
                      .filter(Boolean)
                      .join(' · ')}
                  </p>
                </div>
        ...
        {!loading && !lessons.length ? (
          <EmptyHint>暂时还没有老师共享教案。</EmptyHint>
        ) : null}
      </ToolCard>
